...
class IllustrativeEncoderEnsemble(NNBase):
	def __init__(self, observation_space, action_space, hidden_size=64, channels=[128, 64], use_actor_linear=True, normalize_obs=False, ensemble_size=1, activation='relu'):
		super().__init__(hidden_size)
		flattened_dim = np.prod(observation_space.shape)
		self.normalize_obs = normalize_obs
		self.ensemble_size = ensemble_size
