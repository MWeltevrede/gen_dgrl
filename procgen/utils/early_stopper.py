...
class EarlyStop:
	...
	"""
	def __init__(self, wait_epochs=1, min_delta=0.1, strict=True, save_best=True):
	...
	def should_stop(self, epoch, mean_return, agent, curr_epochs, save_path, env_name, xpid):
		if mean_return > self.best_mean_return + self.delta:
			...
			if self.save_best:
				agent.save(num_epochs=curr_epochs, path=os.path.join(save_path, env_name, xpid, "best_model.pt"))
		else:
			self.waited_epochs += 1
