...
if args.env_name == "control_illustrative_base_c2":
	...
	with open('datasets/rot_invariance_c4.pl', 'rb') as file:
		rotational_inv_taskset = dill.load(file)
	...
	with open('datasets/rot_invariance_base.pl', 'rb') as file:
		rotational_inv_taskset = dill.load(file)
...
# Evaluate rotational invariance
if "control_illustrative_base_c" in args.env_name:
	obs = []
	env = gym.make('ControlIllustrativeCMDP-v0', tasks=rotational_inv_taskset)
	for _ in range(len(rotational_inv_taskset)):
		obs.append(env.reset())
	obs = np.array(obs)
	obs = torch.as_tensor(obs, device=device)
	with torch.no_grad():
		output = agent.eval_step(obs, eps=0)
	total_variation = np.trace(np.cov(output, rowvar=False))
else:
	total_variation = []
	for tasksets in rotational_inv_taskset:
		obs = []
		env = gym.make('ControlIllustrativeCMDP-v0', tasks=tasksets)
